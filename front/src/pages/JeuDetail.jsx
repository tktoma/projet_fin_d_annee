...
import {
    ...
    Eye, Users, Library, CheckCircle, AlertCircle
} from 'lucide-react';
...
function ReportModal({ titre, onClose, onSubmit }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // l'erreur est gérée par le parent, on ne ferme pas le modal
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="bg-secondary-black border border-gray-800 rounded-xl p-5 mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                {[
                    { icon: Eye,     label: 'Consultations', val: jeu.vues || 0 },
                    { icon: Library, label: 'En collection', val: total },
                    { icon: Users,   label: 'Terminé',       val: statuts['FINIT'] || 0, span: true },
                ].map(({ icon: Icon, label, val, span }) => (
                    <div
                        key={label}
                        className={`bg-accent-black rounded-xl p-3 text-center ${
...
function AvisCard({ avis, isOwn, onLike, onDelete, isAuth, onReport }) {
    ...
    // État local pour feedback après signalement
    const [reported, setReported]     = useState(false);
    const [reportError, setReportError] = useState('');

    const handleReportSubmit = async (raison, details) => {
        setReportError('');
        await onReport(avis.id, raison, details);
        // Si onReport ne lève pas d'exception, on marque comme signalé
        setReported(true);
        setReportOpen(false);
    };
    ...
    return (
        <div className="bg-secondary-black border border-gray-800 rounded-xl p-4">
            ...
            <div className="flex items-center gap-3">
                ...
                {/* Actions si connecté et pas son propre avis */}
                {isAuth && !isOwn && (
                    <>
                        {/* Like */}
                        <button
                            onClick={() => onLike(avis.id, true)}
                        ...
                        {/* Signalement — remplacé par confirmation une fois signalé */}
                ...
                {/* Suppression si c'est son propre avis */}
                {isOwn && (
                    <button
...
export const JeuDetail = () => {
    ...
    // État signalement du jeu lui-même
    const [jeuReported, setJeuReported]     = useState(false);
    ...
    // Fermer le menu bibliothèque au clic extérieur
    useEffect(() => {
    ...
    /**
     * Signalement d'un avis depuis AvisCard.
     * Lance la requête API et retourne une promesse.
     * En cas d'erreur, affiche un toast sans bloquer l'appelant.
     */
    const handleReportAvis = async (avisId, raison, details) => {
        try {
            ...
            throw err; // re-throw pour que AvisCard sache qu'il y a eu une erreur
        }
    };

    /**
     * Signalement du jeu lui-même.
     */
        ...
        <div className="min-h-screen bg-primary-black">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex flex-col sm:flex-row gap-6 mb-8">
                    {/* Cover */}
                    <div className="w-40 h-56 flex-shrink-0 rounded-xl overflow-hidden
                                    bg-accent-black self-start">
                        {jeu.coverUrl
                            ? <img src={jeu.coverUrl} alt={jeu.titre}
                                   className="w-full h-full object-cover" />
                            : <div className="w-full h-full flex items-center justify-center
                                              text-5xl text-gray-700">🎮</div>
                        }
                    </div>

                    <div className="flex-1 min-w-0">
                        {/* Titre + bouton signaler le jeu */}
                        <div className="flex items-start justify-between gap-3 mb-2">
                            ...
                            {isAuth && (
                                <div className="flex-shrink-0 mt-1">
                                    {jeuReported ? (
                                        ...
                                        <button
                                            onClick={() => setJeuReportOpen(true)}
                                            className="flex items-center gap-1.5 px-2.5 py-1.5
                                                       rounded-lg border border-gray-700 text-gray-500
                                                       text-xs font-medium hover:border-orange-700
                                                       hover:text-orange-400 transition-colors"
                                        >
                                            <Flag className="w-3 h-3" />
                        ...
                        {/* Badges source / genre / plateforme / année */}
                            ...
                            <div className="relative inline-block">
                                {bibStatus ? (
                                    ...
                                    <>
                                        ...
                                        {showBibMenu && (
                                            <div
                                                className="absolute top-full left-0 mt-1 z-10 w-48
                                                           bg-secondary-black border border-gray-700
                ...
                <div>
                        ...
                        <form onSubmit={handleSendAvis} className="mb-6">
                            <div className="bg-secondary-black border border-gray-800 rounded-xl p-4">
                                <textarea
                                    ...
                                    rows={3}
                                    placeholder="Partagez votre avis… (10 caractères min.)"
                                ...
                                <div className="flex items-center justify-between mt-2 pt-2
                                    ...
                                    <span className={`text-xs ${
                                        monAvis.length < 10 ? 'text-gray-600' : 'text-gray-400'
                                    }`}>
                                        {monAvis.length}/2000
                                    </span>
                                    <button
                                        type="submit"
                                        disabled={sendingAvis || monAvis.length < 10}
                    ...
                    {!isAuth && (
                        <div className="mb-6 p-4 bg-secondary-black border border-gray-800
                                        rounded-xl text-sm text-gray-500">
                            <Link to="/connexion" className="text-primary-red hover:underline">
                                Connectez-vous
                            </Link>{' '}
